...
function createDexGui()
    local DexGui = CreateInstance("ScreenGui", {
        DisplayOrder = 0,
        Enabled = true,
        ResetOnSpawn = true,
        Name = "Dex"
    ...
    local DexGui24 = CreateInstance("Folder", {
        Name = "PropControls",
        Parent = DexGui16
    })
            ...
            for i, v in pairs(newMt.Markers) do
                if i < newMt.TotalSpace then
                    CreateInstance("Frame", {
                        ...
                        BackgroundColor3 = v,
                        BorderSizePixel = 0,
                        Position = hor and UDim2.new(i / newMt.TotalSpace, 0, 1, -6) or
                            UDim2.new(1, -6, i / newMt.TotalSpace, 0),
        ...
        scrollThumbFrame.InputBegan:Connect(function(input)
            ...
            local function doTick()
                ...
                local pos = thumbFrameDist +
                                (mouse[dir] < scrollThumb.AbsolutePosition[dir] +
                                    math.floor(scrollThumb.AbsoluteSize[dir] / 2) and -50 or 50)
                if pos > thumbFrameSize then
                    pos = thumbFrameSize
                elseif pos < 0 then
                    pos = 0
                ...
                if pos < thumbFrameDist and scrollThumbFrame.AbsolutePosition[dir] + pos +
                    ...
                    pos = mouse[dir] - scrollThumbFrame.AbsolutePosition[dir] -
            ...
            for i = 1, entrySpace do
                ...
                if node then
                    ...
                    if not entry then
                        ...
                        entry.Position = UDim2.new(0, self.OffX, 0,
    ...
    function ContextMenu.new()
        ...
        local function show(self, displayFrame, x, y)
            ...
            if reverseY then
                ...
                mainFrame:TweenSize(UDim2.new(0, self.Width, 0, toSize), Enum.EasingDirection.Out,
...
function f.buildPanes()
    ...
    for i, v in pairs(RPaneItems) do
        v.Window:TweenSizeAndPosition(UDim2.new(0, explorerSettings.RPaneWidth, v.Proportion, 0),
            UDim2.new(0, 0, f.prevProportions(RPaneItems, i - 1), 0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart,
            0.5, true)
...
function f.removeFromPane(window)
    ...
    if pane and #pane > 0 then
        local weightTop, weightBottom = 0, 0
...
function f.hookWindowListener(window)
    ...
    window.TopBar.InputBegan:connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            local dragOffX, dragOffY = mouse.X - window.TopBar.AbsolutePosition.X,
                mouse.Y - window.TopBar.AbsolutePosition.Y
            local inPane = false
            local releaseEvent
            local mouseEvent
    ...
    window.InputBegan:connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            for i, v in pairs(activeWindows) do
                if v ~= window then
                    if f.checkMouseInGui(v) and not f.checkInPane(v) then
                        return
                    end
                end
            end
...
function f.newExplorer()
    ...
    newTree.NodeCreate = function(self, entry, i)
        ...
        entry.Indent.Expand.MouseEnter:Connect(function()
            local node = self.Tree[i + self.Index]
            if node then
                if (not self.SearchResults and self.Expanded[node]) or
                    (self.SearchResults and self.SearchExpanded[node.Obj]) then
    ...
    newTree.NodeDraw = function(self, entry, node)
        ...
        if #node > 0 then
            ...
            if (not self.SearchResults and self.Expanded[node]) or
                (self.SearchResults and self.SearchExpanded[node.Obj] == 2) then
                f.icon(entry.Indent.Expand, iconIndex.NodeExpanded)
            else
                f.icon(entry.Indent.Expand, iconIndex.NodeCollapsed)
            end
    ...
    local function fillTree(node, tree)
        ...
        for i = 1, #node do
            ...
            if not isSearching or (isSearching and isSearching[node[i].Obj]) then
                ...
                if (not isSearching and explorerTree.Expanded[node[i]]) or
                    (isSearching and searchExpanded[node[i].Obj] == 2) then
...
local propControls = {
    ...
    ["Vector3"] = function(prop, child)
        ...
        newMt.Children = {f.getChildProp(prop, {
            Name = "X",
            ValueType = "double",
            Depth = 2
        }), f.getChildProp(prop, {
...
local function welcomePlayer()
    ...
    contentR:TweenPosition(UDim2.new(1, -explorerSettings.RPaneWidth, 0, 0), Enum.EasingDirection.Out,
        Enum.EasingStyle.Quart, 0.5, true)
