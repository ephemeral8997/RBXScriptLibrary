...
profile("Inf Yield", function()
    ...
    Tooltip = Instance.new("Frame") -- 5ms (ended)
    ...
    task.spawn(function()
        ...
        local success, result = pcall(function()
            chatbox = game.WaitForChild(PlayerGui, "Chat").Frame.ChatBarParentFrame.Frame.BoxFrame.Frame.ChatBar
        end)
        if success then
    ...
    function addcmd(name, alias, func, plgn, desc)
        cmds[#cmds + 1] = {
            NAME = name,
            ALIAS = alias or {},
            FUNC = func,
    ...
    addcmd("snaptocframe", { "forcetp", "godmove" }, function(args, speaker)
        ...
        local components = string.split(arg)
    ...
    addcmd("dupetools", { "clonetools" }, function(args, speaker)
        ...
        for i = 1, LOOP_NUM do
            ...
            Human.RootPart.Anchored = speaker:ClearCharacterAppearance(task.wait(0.1)) or true
    ...
    addcmd("givetool", { "givetools" }, function(args, speaker)
        ...
        speaker:ClearCharacterAppearance()
        task.spawn(function()
            speaker.CharacterAdded:Wait():WaitForChild("Humanoid").RootPart.CFrame = task.wait() and hrppos
        end)
