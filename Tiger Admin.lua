...
function API:Notif(Text, Dur)
    task.spawn(function()
        ...
        API:Tween(Frame_1, "BackgroundTransparency", 0.350, 0.5)
        API:Tween(TextLabel, "TextTransparency", 0, 0.5)
        wait(Dur + 0.7)
        API:Tween(Frame_1, "BackgroundTransparency", 1, 0.5)
        API:Tween(TextLabel, "TextTransparency", 1, 0.5)
        wait(0.7)
        Notif:Destroy()
    end)
    return
    ...
    if not IsBringing and Target and Target.Character:FindFirstChildOfClass("Humanoid") and
        ...
        repeat
            ...
            if Target.Character:FindFirstChildOfClass("Humanoid").MoveDirection.Magnitude > 0 then
                car:SetPrimaryPartCFrame(Target.Character:GetPrimaryPartCFrame() * CFrame.new(0, -0.2, -6))
            else
                car:SetPrimaryPartCFrame(Target.Character:GetPrimaryPartCFrame() * CFrame.new(0, -0.2, -5))
            end
        ...
        task.spawn(function()
            if PremiumActivated and not DontBreakCar then
                repeat
                    task.wait()
                until Target.Character:FindFirstChildOfClass("Humanoid").Sit == false
                repeat
                    wait()
                    Seat:Sit(Player.Character:FindFirstChildOfClass("Humanoid"))
                until Player.Character:FindFirstChildOfClass("Humanoid").Sit == true
...
function API:ChangeTeam(TeamPath, NoForce, Pos)
    ...
    if TeamPath == game.Teams.Criminals then
        ...
        repeat
            API:swait()
        until Player.Team == game.Teams.Inmates and Player.Character:FindFirstChild("HumanoidRootPart")
        repeat
            API:swait()
...
function API:KillPlayer(Target, Failed, DoChange)
    ...
    repeat
        API:swait()
    until not Target.Character:FindFirstChildOfClass("ForceField")
    ...
    repeat
        API:swait()
        Gun = Player.Backpack:FindFirstChild("Remington 870") or Player.Character:FindFirstChild("Remington 870")
    until Gun
    task.spawn(function()
...
function API:killall(TeamToKill)
    ...
    elseif TeamToKill then
        ...
        repeat
            task.wait()
            Gun = Player.Backpack:FindFirstChild("M9") or Player.Character:FindFirstChild("M9")
        until Gun
...
do
                        ...
                        pcall(function()
                            ...
                            if target ~= nil then
                                ...
                                if (target.Position - player.Character.Head.Position).magnitude < 5 then
                                    ...
                                    for i = 1, 3 do
                                        game.ReplicatedStorage.meleeEvent:FireServer(game.Players[target.Parent.Name])
                                    end
                                end
                            else
        ...
        API:CreateCmd("grabknife", "Cool script [REJOIN TO STOP]", function(args)
            task.spawn(function()
                ...
                local function Slash()
                    ...
                    SetPos(CFrame.new(-0.1, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)), true)
                    task.wait()
                    SetPos(CFrame.new(-0.2, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)))
                    task.wait()
                    SetPos(CFrame.new(-0.3, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)), true)
                    task.wait()
                    SetPos(CFrame.new(-0.4, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)))
                    task.wait()
                    SetPos(CFrame.new(-0.5, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)), true)
                    task.wait()
                    SetPos(CFrame.new(-0.6, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)), true)
                    task.wait()
                    SetPos(CFrame.new(-0.7, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)), true)
                    task.wait()
                    SetPos(CFrame.new(-0.8, -2.1, 0.5), CFrame.new(1.5, 0, 0) * CFrame.Angles(0, 0, math.rad(-90)), true)
                    task.wait()
                    wait(0.5)
                    StopAnimations()
                    Idle()
    ...
    API:CreateCmd("tase", "Tases the target", function(args)
        if args[2] == "all" then
            ...
            repeat
                task.wait()
            until Player.Backpack:FindFirstChild("Taser")
            wait(0.7)
            local ohTable1 = {}
            for i, v in pairs(game:GetService("Players"):GetPlayers()) do
            ...
            for i, v in pairs(game:GetService("Players"):GetPlayers()) do
                if v and v ~= plr and not table.find(API.Whitelisted, v) then
                    ...
                    v.CharacterAdded:Connect(function(c)
                        repeat
                            task.wait()
                        until c and c:FindFirstChildOfClass("Humanoid")
                        if States.AntiPunch then
                            if States.antipunch then
                                AntiPunchC(v)
                            end
    ...
    API:CreateCmd("unload", "Destroys the script unloading it", function(args)
        ...
        Temp = {}
        game:GetService("Workspace").Camera.CameraSubject = plr.Character.Humanoid
        CmdBarFrame:TweenPosition(CmdBarFrame.Position - UDim2.new(0, 0, -0.5, 0), "Out", "Back", 0.8)
        wait(1)
        ScreenGui:Destroy()
    end)
end
...
local blackperson
blackperson = hookfunction(game.Destroy, function(a)
...
CmdBarFrame:TweenPosition(UDim2.new(0.5, 0, 0.899999998, 0) - UDim2.new(0, 0, 0.05, 0), "Out", "Back", 0.5)
